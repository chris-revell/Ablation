...
# for ablatedCell in testCells
    ...
    lines!(axes[2], binMidpoints, smoothedData, color=:green)
    ...
    derivys = []
    for i=1:length(xs)
        push!(ys, exponentialDeriv(xs[i], sol.u))
    end
    ...
    lines!(polarAxis, [sum(intersectsSmoothed), sum(intersectsSmoothed), sum(intersectsSmoothed)], [1.0,0.0,1.0], color=:black)

    # ylims!(axes[2], (minimum(directionsSorted),maximum(directionsSorted)))
    ylims!(axes[2], (-1.0,1.0))
    # ylims!(axes[2], (minimum(directionsSorted),maximum(directionsSorted)))
    # ylims!(axes[2], (-1.0,1.0))
    # # ylims!(axes[2], (minimum(directionsSorted),maximum(directionsSorted)))
    # ylims!(axes[2], (-1.0,1.0))
...
function ϕ(r, θ, θ₀, a, b)
    if abs(a/b + cos(2*(θ-θ₀))) < 1.0
        return a/b + cos(2*(θ-θ₀))
    else
        return sign(a/b + cos(2*(θ-θ₀)))*1.0
    end
end

predictedDirection = ϕ.(norm.(radiusVectorsSorted), θsSorted, divθ+π/2, aOverb, 1.0)

lines!(axes[2], θsSorted, predictedDirection)
