...
for (col,inputSystem) in enumerate(inputSystems)
    ...
    for i=1:I
        poly!(axes[end],cellPolygons[i],color=cocurlᶜh[i],colorrange=row1Lims,colormap=:bam,strokewidth=1,strokecolor=(:black,0.25))
    end
    ...
    Colorbar(gl[1,3], colorrange=row1Lims, colormap=:bam, height=Relative(0.8))

    push!(axes, Axis(gl[2,1], aspect=DataAspect()))
    for k=1:K
        poly!(axes[end],linkTriangles[k],color=-divᵛh[k],colorrange=row2Lims,colormap=:bam,strokewidth=1,strokecolor=(:white,0.0))
    end
    for i=1:I
    ...
    for k=1:K
        poly!(axes[end],linkTriangles[k],color=cocurlᵛh[k],colorrange=row2Lims,colormap=:bam,strokewidth=1,strokecolor=(:white,0.0))
    end
    for i=1:I
        poly!(axes[end],cellPolygons[i],color=(:white,0.0),strokewidth=1,strokecolor=(:black,0.25))
    end
    ...
    for i=1:I
        poly!(axes[end],cellPolygons[i],color=curlᶜh[i],colorrange=row3Lims,colormap=:bam,strokewidth=1,strokecolor=(:black,0.25))
    end
    Label(gl[3,1,Bottom()], L"\mathrm{curl}^c",fontsize=36)
    push!(axes, Axis(gl[3,2], aspect=DataAspect()))
    for i=1:I
        poly!(axes[end],cellPolygons[i],color=codivᶜh[i],colorrange=row3Lims,colormap=:bam,strokewidth=1,strokecolor=(:black,0.25))
    end
    Label(gl[3,2,Bottom()], L"\mathrm{codiv}^c",fontsize=36)
    Colorbar(gl[3,3], colorrange=row3Lims, colormap=:bam, height=Relative(0.8))
    ...
    for k=1:K
        poly!(axes[end],linkTriangles[k],color=curlᵛh[k],colorrange=row4Lims,colormap=:bam,strokewidth=1,strokecolor=(:white,0.0))
    end
    for i=1:I
        poly!(axes[end],cellPolygons[i],color=(:white,0.0),strokewidth=1,strokecolor=(:black,0.25))
    end
...
save(plotsdir(inputDir, "figureDerivatives.png"), fig)
# save(plotsdir("figureDerivatives_$(suppressOrNot)_$(spokesOrNot).pdf"), fig)
save(plotsdir(inputDir, "figureDerivatives.pdf"), fig)