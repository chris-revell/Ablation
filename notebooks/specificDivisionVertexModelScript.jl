...
eigenVals, eigenVecs = eigen(matrices1.cellShapeTensor[ind]) # eigenvalues/vectors listed smallest to largest eigval.
if eigenVecs[:,1][2] < 0.0 #make it so vector is pointing in positive y direction (to fit with existing code in assigning new edges)
    shortvec = -1.0*matrices1.cellPerimeters[ind].*eigenVecs[:,1] # Multiplication by cell perimeter ensures this axis is long enough to completely cross the cell; eigenvector has unit length otherwise
else
    shortvec = matrices1.cellPerimeters[ind].*eigenVecs[:,1] # Multiplication by cell perimeter ensures this axis is long enough to completely cross the cell; eigenvector has unit length otherwise
end
shortAxisLine = Line(Point{2,Float64}(matrices1.cellPositions[ind].+shortvec), Point{2,Float64}(matrices1.cellPositions[ind].-shortvec))

# Test cell edges for an intersection
edgeLines = Line.(Point{2,Float64}.(R1[matrices1.cellVertexOrders[ind][0:end-1]]), Point{2,Float64}.(R1[matrices1.cellVertexOrders[ind][1:end]])) # Start and end with the same vertex by indexing circular array from 0 to end
...
intersectedEdges = matrices1.cellEdgeOrders[ind][intersectedIndices]

newCellVertices = matrices1.cellVertexOrders[ind][intersectedIndices[1]:intersectedIndices[2]-1] # Not including new vertices to be added later
oldCellVertices = setdiff(matrices1.cellVertexOrders[ind], newCellVertices) # Not including new vertices to be added later
newCellEdges = matrices1.cellEdgeOrders[ind][intersectedIndices[1]+1:intersectedIndices[2]-1] # IntersectedEdges allocated to old cell, not including new edge to be added later
oldCellEdges = setdiff(matrices1.cellEdgeOrders[ind], newCellEdges) # Not including new edge to be added later
...
Btmp[1:params1.nCells ,1:params1.nEdges] .= matrices1.B            
...
if matrices1.boundaryEdges[intersectedEdges[1]] == 0
    neighbourCell = setdiff(findall(x->x!=0, @view matrices1.B[:,intersectedEdges[1]]),[ind])[1]
    Btmp[neighbourCell,newEdges[2]] = -1
end
if matrices1.boundaryEdges[intersectedEdges[2]] == 0
    neighbourCell = setdiff(findall(x->x!=0, @view matrices1.B[:,intersectedEdges[2]]),[ind])[1]
...
for edge in [newCellEdges...]
    if matrices1.boundaryEdges[edge] == 0
        neighbourCell = setdiff(findall(x->x!=0, @view matrices1.B[:,edge]),[ind])[1]
...
Atmp[1:params1.nEdges,1:params1.nVerts] .= matrices1.A

# First intersected old edge (which remains in the old cell) loses downstream vertex and gains the new vertex
Atmp[intersectedEdges[1],matrices1.cellVertexOrders[ind][intersectedIndices[1]]] = 0
Atmp[intersectedEdges[1],newVertices[1]] = matrices1.A[intersectedEdges[1],matrices1.cellVertexOrders[ind][intersectedIndices[1]]]

# Second intersected old edge (which remains in the old cell) loses upstream vertex and gains the new vertex             
Atmp[intersectedEdges[2],matrices1.cellVertexOrders[ind][intersectedIndices[2]-1]] = 0
Atmp[intersectedEdges[2],newVertices[2]] = matrices1.A[intersectedEdges[2],matrices1.cellVertexOrders[ind][intersectedIndices[2]-1]]
...
Atmp[newEdges[2],matrices1.cellVertexOrders[ind][intersectedIndices[1]]] = 1

# Third new edge gains second new vertex downstream (1) and upstream vertex of second old intersected edge upstream (-1)
Atmp[newEdges[3],matrices1.cellVertexOrders[ind][intersectedIndices[2]-1]] = -1
...
R_in = copy(R1)
push!(R_in, SVector{2,Float64}(intersections[intersectedIndices[1]][2]))
push!(R_in, SVector{2,Float64}(intersections[intersectedIndices[2]][2]))
...
push!(axes, Axis(fig))
scatter!(axes[end], log10.(radii[Not(ind)]), log10.(cellDisplacementNorms[Not(ind)]), color=arrowColours[Not(ind)], colorrange=clims, colormap=:bwr)
...
for i = 1:params2.nCells-1
    if i!=ind 
        poly!(axes[end], cellPolygons2[i], color=arrowColours[i], colorrange=(-1.0,1.0), colormap=:bwr, strokecolor=(:black, 0.2), strokewidth=1)
    end
end
...
# arrows!(axes[end], Point{2,Float64}.(matrices1.cellPositions[Not(ind)]), Vec{2,Float64}.(cellDisplacements[Not(ind)]), color=arrowColours[Not(ind)], colorrange=clims, colormap=:bwr, lengthscale=20.0, linewidth=5)
...
push!(axes, Axis(fig))
scatter!(axes[end], log10.(radii[Not(ind)]), log10.(cellDisplacementNorms), color=arrowColours, colorrange=clims, colormap=:bwr)
xs = log10.(norm.(radiusVectors[Not(ind)]))
axes[end].xlabel = L"\log_{10}\left(R_i\right)"
axes[end].ylabel = L"\log_{10}\left(\Delta R_i\right)"
# ys = -1.5.-1.0.*xs
...
push!(axes, Axis(fig, aspect=DataAspect()))
for i = 1:params3.nCells
    poly!(axes[end], cellPolygons3[i], color=arrowColours[i], colorrange=(-1.0,1.0), colormap=:bwr, strokecolor=(:black, 0.2), strokewidth=1)
end
# arrows!(axes[end], Point{2,Float64}.(matrices3.cellPositions), Vec{2,Float64}.(cellDisplacements), color=arrowColours, colorrange=clims, colormap=:bwr, lengthscale=20.0, linewidth=5)
