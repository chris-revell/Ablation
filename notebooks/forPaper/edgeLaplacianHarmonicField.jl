...
arrowColours = [(:blue, norm(v)/maximum(norm.(edgeVectors))) for v in edgeVectors]
arrows!(axes[end], Point{2,Float64}.(𝐂ⱼ[peripheralEdges.==0]), Vec{2,Float64}.(edgeVectors[peripheralEdges.==0]), color=arrowColours[peripheralEdges.==0], linewidth=2, lengthscale=1.0)
arrows!(axes[end], Point{2,Float64}.(𝐜ⱼ[peripheralEdges.!=0]), Vec{2,Float64}.(edgeVectors[peripheralEdges.!=0]), color=arrowColours[peripheralEdges.!=0], linewidth=2, lengthscale=1.0)
