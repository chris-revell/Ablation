...
for (col,inputSystem) in enumerate(inputSystems)
    ...
    for k=1:Kⁱ
        poly!(axes[end],linkTriangles[k],color=-divᵛh[k],colorrange=divᵛhLims,colormap=:bam,strokewidth=1,strokecolor=(:white,0.0))
    end
    for i=1:I
    ...
    for k=1:Kⁱ
        poly!(axes[end],linkTriangles[k],color=codivᵛh[k],colorrange=codivᵛhLims,colormap=:bam,strokewidth=1,strokecolor=(:white,0.0))
    end
    for i=1:I
        poly!(axes[end],cellPolygons[i],color=(:white,0.0),strokewidth=1,strokecolor=(:black,0.25))
    end
...
hidespines!.(axes)
display(fig)
save(plotsdir(inputDir, "midpointsfigureDerivatives.png"), fig)
save(plotsdir(inputDir, "midpointsfigureDerivatives.pdf"), fig)